...
let parseInput = input => {
  let resp = Array.of_list(String.split_on_char('\n', input));

  let firstLine = resp[0];
  let secondLine = resp[1];

  let parsedFirstLine = Array.of_list(String.split_on_char(',', firstLine));
  let parsedSecondLine =
    Array.of_list(String.split_on_char(',', secondLine));
